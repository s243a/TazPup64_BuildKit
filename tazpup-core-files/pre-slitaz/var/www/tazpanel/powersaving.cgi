...
menu)
cat <<EOT
<li><a data-icon="" href="powersaving.cgi" data-root>$(_ 'Power saving')</a></li>
EOT
exit
esac
header
...
<section>
<header>
<span data-icon="">DPMS (Display Power Management Signaling)</span>
</header>
...
printf "<tr><td data-icon=\"\">%s</td><td>%s</td><td>%s</td><td>", I, V, M;
...
<a data-icon="" href="index.cgi?file=$monitor_conf">$(basename $monitor_conf)</a><br/>
<a data-icon="" href="index.cgi?file=$layout_conf">$(basename $layout_conf)</a>
</fieldset>
...
cat <<EOT
<section>
<header><span data-icon="">$(_ 'CPU')</span></header>
...
if echo $lsmod | grep -q " $module "; then icon=''; else icon=''; fi
