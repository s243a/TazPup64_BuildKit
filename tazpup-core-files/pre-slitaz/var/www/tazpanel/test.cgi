...
cat <<EOT
<section>
<header><span data-img=""></span>Buttons with font icons</header>
EOT
icons=":add :admin :back :battery :brightness \
:cancel :cd :check :clock :conf :daemons :delete \
:detect :diff :download :edit :eth :group :grub \
:hdd :help :history :info :install :link :list \
:locale :lock :logs :loopback :man :modules :off \
:ok :on :opt :proc :refresh :removable :remove \
:repack :report :restart :run :save :scan \
:settings :slitaz :start :stop :sync :tags :tag \
:tazx :temperature :terminal :text :unlink \
:unlock :upgrade :user :view :web :wifi :toggle \
:chlock :calendar :modem :vpn :display :cpu \
:floppy :folder"
echo "$icons" | \
awk 'BEGIN{RS=" "; FS=":"} {
...
<section>
<header><span data-img=""></span>Links with font icons</header>
<div>
<p>
EOT
echo "$icons" | \
...
<section>
<header><span data-img=""></span>Links with font icons only (small buttons)</header>
<p>
EOT
echo "$icons" | \
awk 'BEGIN{RS=" "; FS=":"} {
...
<section>
<header><span data-img=""></span>Status icons</header>
<div>
<span data-icon="">lvl0</span> <span data-icon="">lvl1</span> <span data-icon="">lvl2</span>
<span data-icon="">lvl3</span> <span data-icon="">lvl4, lvl5</span>
<span data-icon="">online</span> <span data-icon="">offline</span>
<span data-icon="">sechi</span> <span data-icon="">secmi</span> <span data-icon="">seclo</span>
<span data-icon="">pkg</span> <span data-icon="">pkgi</span> <span data-icon="">pkgib</span>
</div>
<div>
<span data-icon="">msg</span> <span data-icon="">msgerr</span>
<span data-icon="">msgwarn</span> <span data-icon="">msgup</span>
<span data-icon="">msgtip</span>
</div>
</section>
<section>
...
<section>
<header><span data-img=""></span>User input elements</header>
<div><form>
<table>
<tr><td>Text:</td>
...
<tr><td>Button:</td>
<td><input type="button" value="Button" data-icon=""/></td>
</tr>
<tr><td>Checkbox:</td>
...
<tr><td>Select:</td>
<td><select><option data-icon="">Option 1<option>Option 2<option>Option 3</select></td>
</tr>
